...
def process_roi(img, points):
    # cv2.imshow('ori', cv2.resize(img, None, fx=0.1, fy=0.1))
    mask = np.zeros_like(img)
    # cv2.imshow('mask A', cv2.resize(mask, None, fx=0.1, fy=0.1))
    if len(img.shape) == 3:
        cv2.fillPoly(mask, [points], (255, 255, 255))
    else:
        cv2.fillPoly(mask, [points], 255)
    # cv2.imshow('mask B',  cv2.resize(mask, None, fx=0.1, fy=0.1))
    masked_img = cv2.bitwise_and(img, mask)
    # cv2.imshow('origin img with ROI', cv2.resize(masked_img, None, fx=0.1, fy=0.1))
    x, y, w, h = cv2.boundingRect(points)
    # cv2.imshow('bounding Rect ROI', cv2.resize(masked_img[y:y + h, x:x + w], None, fx=0.2, fy=0.2))
    # cv2.waitKey()
    # cv2.destroyAllWindows()
    return masked_img[y:y + h, x:x + w]

def process_roi2(img, points):
    x, y, w, h = cv2.boundingRect(points)
...
def image_processed(img, points):
    ...
    # 形態學腐蝕，獲得整體切屑輪廓
    # good
    # eroded = cv2.erode(b, cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2)), iterations=1)
    ...
    # 膨脹
    dilated = cv2.dilate(eroded, cv2.getStructuringElement(cv2.MORPH_RECT, (10, 10)), iterations=3)
...
if __name__ == '__main__':
    ...
    ps = np.array([[560, 2539], [1550, 2504], [2053, 5699], [880, 5982]])
    image_processed(image, p2)
    # process_roi2(image, ps)
