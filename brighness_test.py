...
def test():
    ...
    for file in files:
        if file.lower().endswith(image_extensions):
            ...
            var = np.mean(process_roi(img, p2))
...
if __name__ == '__main__':
    ...
    test()
    # img = cv2.imread('photo_20240718_153843.png', cv2.IMREAD_GRAYSCALE)
    # cv2.imshow('roi', cv2.resize(process_roi(img, p2), None, fx=0.1, fy=0.1))
    # cv2.waitKey()
    # cv2.destroyAllWindows()
